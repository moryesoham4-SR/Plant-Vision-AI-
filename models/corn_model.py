...
def predict_corn(tensor_input: np.ndarray, image_hint: str = "") -> Dict[str, Any]:
    if config.USE_MOCK:
        return get_mock_prediction("corn", image_hint)

    from models.model_loader import get_or_load_model
    model = get_or_load_model("corn")
    ...
    classes = config.PLANTS["corn"]["classes"]
    predicted_disease = classes[class_idx]
    disease_info = DISEASE_KNOWLEDGE_BASE["corn"].get(predicted_disease, {})

    probabilities = {classes[i]: float(predictions[0][i]) for i in range(len(classes))}

    return {
        "plant_id": "corn",
        "plant_name": config.PLANTS["corn"]["name"],
        "scientific_name": config.PLANTS["corn"]["scientific_name"],
