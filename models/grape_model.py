...
def predict_grape(tensor_input: np.ndarray, image_hint: str = "") -> Dict[str, Any]:
    if config.USE_MOCK:
        return get_mock_prediction("grape", image_hint)

    from models.model_loader import get_or_load_model
    model = get_or_load_model("grape")
    ...
    classes = config.PLANTS["grape"]["classes"]
    predicted_disease = classes[class_idx]
    disease_info = DISEASE_KNOWLEDGE_BASE["grape"].get(predicted_disease, {})

    probabilities = {classes[i]: float(predictions[0][i]) for i in range(len(classes))}

    return {
        "plant_id": "grape",
        "plant_name": config.PLANTS["grape"]["name"],
        "scientific_name": config.PLANTS["grape"]["scientific_name"],
